...
void CTitle::Init(){
	...
	for (int i = 0; i < 5; i++){
		mTitleNum[i].mSize[2] = 0 - (i * 64);
		mTitleNum[i].mSize[3] = 48 - (i * 64);
...
void CTitle::Update(){
	...
	if ((CGamePad::OncePush(PAD_DOWN) || CGamePad::OncePush(PAD_LSTICKY, -0.5f) || CKey::OncePush(VK_DOWN) || CKey::OncePush('S')) && cursor_num < CSceneChange::ESIZE - 2){
		cursor_num++;
