...
void CText::DrawTextW(wchar_t a, float left, float right, float bottom, float top,float r, float g, float b, float alpha, int font){
	...
	else if (a >= L'▼' && a <= L'▽'){		//U+25BC	下向き, U+25BD	左向き
		a = a - L'▼';
	...
	else if (a == L'終'){					//U+7D42
		CText::uv[0] = 1 * CSIZE;
		CText::uv[1] = CText::uv[0] + CSIZE;
		CText::uv[2] = CSIZE * 11;
		CText::uv[3] = CSIZE * 10;
	}
	else if (a == L'了'){					//U+4E86
		CText::uv[0] = 2 * CSIZE;
		CText::uv[1] = CText::uv[0] + CSIZE;
		CText::uv[2] = CSIZE * 11;
		CText::uv[3] = CSIZE * 10;
	}
	else if (a == L'素'){					//U+7D20
		CText::uv[0] = 3 * CSIZE;
		CText::uv[1] = CText::uv[0] + CSIZE;
		CText::uv[2] = CSIZE * 11;
		CText::uv[3] = CSIZE * 10;
	}
	else if (a == L'材'){					//U+6750
		CText::uv[0] = 4 * CSIZE;
		CText::uv[1] = CText::uv[0] + CSIZE;
		CText::uv[2] = CSIZE * 11;
		CText::uv[3] = CSIZE * 10;
	}
	else if (a == L'借'){					//U+501F
		CText::uv[0] = 5 * CSIZE;
		CText::uv[1] = CText::uv[0] + CSIZE;
		CText::uv[2] = CSIZE * 11;
		CText::uv[3] = CSIZE * 10;
	}
	else if (a == L'用'){					//U+7528
		CText::uv[0] = 6 * CSIZE;
		CText::uv[1] = CText::uv[0] + CSIZE;
		CText::uv[2] = CSIZE * 11;
		CText::uv[3] = CSIZE * 10;
	...
	/*------ 魔王魂 ------*/
	else if (a == L'魔'){					//U+9B54
		CText::uv[0] = 7 * CSIZE;
		CText::uv[1] = CText::uv[0] + CSIZE;
		CText::uv[2] = CSIZE * 11;
		CText::uv[3] = CSIZE * 10;
	}
	else if (a == L'王'){					//U+738B
		CText::uv[0] = 8 * CSIZE;
		CText::uv[1] = CText::uv[0] + CSIZE;
		CText::uv[2] = CSIZE * 11;
		CText::uv[3] = CSIZE * 10;
	}
	else if (a == L'魂'){					//U+9B42
		CText::uv[0] = 9 * CSIZE;
		CText::uv[1] = CText::uv[0] + CSIZE;
		CText::uv[2] = CSIZE * 11;
		CText::uv[3] = CSIZE * 10;
	...
	else if (a == L'教'){					//U+6559
		CText::uv[0] = 29 * CSIZE;
		CText::uv[1] = CText::uv[0] + CSIZE;
		CText::uv[2] = CSIZE * 12;
		CText::uv[3] = CSIZE * 11;
	}
	else if (a == L'員'){					//U+54E1
		CText::uv[0] = 30 * CSIZE;
		CText::uv[1] = CText::uv[0] + CSIZE;
		CText::uv[2] = CSIZE * 12;
		CText::uv[3] = CSIZE * 11;
